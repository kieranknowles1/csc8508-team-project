...
vec3 DepthToWorldPosition(vec2 texCoords, double depth) {
...
vec3 calculateNormal(vec2 texCoord){
    ...
    double depthRight = texture(depthTex, texCoord + vec2(offset_x, 0)).r;
    double depthUp = texture(depthTex, texCoord + vec2(0, offset_y)).r;
...
void main(void) {
   ...
   for (int i = 0; i < 25; i++) {
       ...
       if(distance(normalWorld,normalWorld2) >= 1.0f){ // normals are substantially different
           colour = vec4(0,0,0,1);
           break;
        }
    }
